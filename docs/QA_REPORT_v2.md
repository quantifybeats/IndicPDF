...
| F12 | **NEW (HIGH) — zip-amplification DoS on the DOCX→PDF conversion path.** `04_large_telugu_padding.docx` (90 KB compressed → **24.5 MB** decompressed) and `max_size_test.docx` (→17.6 MB) pass the 25 MB *compressed* upload cap, then the FPDF fallback renders run-by-run for minutes and head-of-line-blocks the single worker (observed: a 5-file batch stalled at 2/5). The reconstruction engine had the F5 guard; the conversion path did not. **Fixed:** `_assert_docx_within_render_limits` rejects >20 MB decompressed `word/document.xml` before python-docx loads it, plus a 5000-paragraph render cap with a `truncated` warning. | New tests `tests/test_docx_decompression_cap.py` (bomb rejected; paragraph cap flagged). |

**Multi-format `/convert` (LibreOffice):** installed LibreOffice 26.2.4; verified DOCX→{pdf, odt, rtf, html, txt, doc} end-to-end — direct engine, live `/convert` HTTP, **and in-browser through the Converter UI's own code path** (5/6 in one burst; the 6th hit the 5/min rate limit, as designed). odt/rtf/html/doc preserve Devanagari+Telugu+Tamil. The frontend Converter was already wired to `/convert`; removed a `pdf→jpg` option the backend can't serve. (`txt` export has a minor LibreOffice encoding quirk.) Added `scripts/dev_serve.sh` + a `.claude/launch.json` "app" entry to run the full stack locally.
...
- **v2.3 (2026-06-11):** New finding F12 (zip-amplification DoS on the DOCX→PDF path) found during multi-file/multi-format load testing — fixed with decompression + paragraph caps and regression tests. Installed LibreOffice and verified all `/convert` formats work end-to-end including through the browser UI. 45 backend + 16 frontend tests pass.
